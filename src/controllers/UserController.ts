...
async function getUserProfileData(req: Request, res: Response): Promise<void> {
  ...
  if (!user) {
    res.redirect('/welcome'); // 404 Not Found
    return;
  }
...
async function deleteAccount(req: Request, res: Response): Promise<void> {
  ...
  const user = await getUserByEmail(email);
  if (!user) {
    res.redirect('/welcome'); // 404 Not Found - email doesn't exist
    return;
  }

  if (authenticatedUser.userId !== user.userId) {
    res.redirect('/welcome'); // trying to delete someone elses account
    return;
  }
  ...
  if (!(await argon2.verify(passwordHash, password))) {
    res.redirect('/welcome'); // 404 not found - user w/ email/password doesn't exist
  }
